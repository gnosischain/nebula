...
- cerebro-mcp may not be able to read `nebula_discv4`. On 2026-09-09 its reader user had no grant on it. Its code allowlist now includes it (cerebro-mcp `src/cerebro_mcp/config.py:469-481`), and the migration plan grants the new reader every analytics database (`WAREHOUSE-MIGRATION.md:195`, in the deployments repo's GKE deployments directory). Try `SELECT 1 FROM nebula_discv4.visits LIMIT 1` first; if it fails, use another warehouse client.
